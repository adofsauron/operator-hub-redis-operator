...
func (opt *Options) AddFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&opt.Development, "development", false, "development flag will allow to run the operator outside a kubernetes cluster")
}
...
func (opt *Options) Config() (*config.Config, error) {
	...
	c := &config.Config{
		...
		StatefulSetTemplate:                redisStandaloneStatefulsetTemp,
